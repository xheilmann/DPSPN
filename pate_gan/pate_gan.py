...
def pategan(x_train, parameters):
  ...
  ## Placeholder
  # PATE labels
  Y = tf.compat.v1.placeholder(tf.compat.v1.float32, shape = [None, 1])
  # Random Variable    
  Z = tf.compat.v1.placeholder(tf.compat.v1.float32, shape = [None, z_dim])
   
  ## NN variables   
  # Student
  S_W1 = tf.compat.v1.Variable(xavier_init([dim, student_h_dim]))
  S_b1 = tf.compat.v1.Variable(tf.compat.v1.zeros(shape=[student_h_dim]))
    
  S_W2 = tf.compat.v1.Variable(xavier_init([student_h_dim,1]))
  S_b2 = tf.compat.v1.Variable(tf.compat.v1.zeros(shape=[1]))

  theta_S = [S_W1, S_W2, S_b1, S_b2]
    
  # Generator

  G_W1 = tf.compat.v1.Variable(xavier_init([z_dim, generator_h_dim]))
  G_b1 = tf.compat.v1.Variable(tf.compat.v1.zeros(shape=[generator_h_dim]))

  G_W2 = tf.compat.v1.Variable(xavier_init([generator_h_dim,generator_h_dim]))
  G_b2 = tf.compat.v1.Variable(tf.compat.v1.zeros(shape=[generator_h_dim]))

  G_W3 = tf.compat.v1.Variable(xavier_init([generator_h_dim,dim]))
  G_b3 = tf.compat.v1.Variable(tf.compat.v1.zeros(shape=[dim]))
    
  theta_G = [G_W1, G_W2, G_W3, G_b1, G_b2, G_b3]

  ## Models
  def generator(z):
    G_h1 = tf.compat.v1.nn.tanh(tf.compat.v1.matmul(z, G_W1) + G_b1)
    G_h2 = tf.compat.v1.nn.tanh(tf.compat.v1.matmul(G_h1, G_W2) + G_b2)
    G_out = tf.compat.v1.nn.sigmoid(tf.compat.v1.matmul(G_h2, G_W3) + G_b3)
        
    return G_out
    
  def student(x):
    S_h1 = tf.compat.v1.nn.relu(tf.compat.v1.matmul(x, S_W1) + S_b1)
    S_out = tf.compat.v1.matmul(S_h1, S_W2) + S_b2
  ...
  S_loss = tf.compat.v1.reduce_mean(Y * S_fake) - tf.compat.v1.reduce_mean((1-Y) * S_fake)
  G_loss = -tf.compat.v1.reduce_mean(S_fake)
  ...
  clip_S = [p.assign(tf.compat.v1.clip_by_value(p, -0.01, 0.01)) for p in theta_S]
