import numpy as np
from mst.graphical_model import GraphicalModel
import mst.callbacks as callbacks
from mst.clique_vector import CliqueVector
...
class FactoredInference:
    def __init__(self, domain, backend = 'numpy', structural_zeros = {}, metric='L2', log=False, iters=1000, warm_start=False, elim_order=None):
        ...
        if backend == 'torch':
            from mst.torch_factor import Factor
            self.Factor = Factor
        else:
            from mst.torch_factor import Factor
            self.Factor= Factor
